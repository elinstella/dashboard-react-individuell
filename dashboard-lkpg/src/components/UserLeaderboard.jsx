...
const UserLeaderboard = ({ usersData }) => {
  ...
  useEffect(() => {
    ...
    if (usersData && usersData.users && Array.isArray(usersData.users)) {
      const sortedUsers = usersData.users
        .map(user => {
          const totalStreams = Object.values(user.monthlyStreams || {}).reduce((sum, streams) => sum + streams, 0);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">User Streams Leaderboard</h1>
      {users.length === 0 ? (
        <p className="text-gray-500">No users to display. Check the console for errors.</p>
      ) : (
            ...
            >
              ...
              <div className="flex-grow">
                <p className="text-lg font-medium text-gray-900">
                  {user.name} <span className="text-gray-500">({user.location})</span>
                </p>
                <p className="text-sm text-gray-600">Total Streams: {user.totalStreams}</p>
              </div>
            </li>
          ))}
        </ul>
